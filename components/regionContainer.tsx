...
const RegionContainer = () => {
  ...
  useEffect(() => {
    query.r ? setRegion(query.r as string) : setRegion(null);
    setOpen(false)
  }, [query])
  ...
  return (
      <div className="w-1/2 lg:w-44 relative" onClick={() => setOpen(prev => !prev)}>
        <button id="dropdownHoverButton" data-dropdown-toggle="dropdownHover" data-dropdown-trigger="hover"
          ...
          <span
              className={`transition-all duration-300 relative w-4 h-4 transform ${isOpen ? "rotate-0" : "rotate-180"}`}><ChevronSVG/></span>
        </button>
...
type RegionProps = { title: string, clickHandler: () => void }
export const RegionItem: FC<RegionProps> = ({title, clickHandler}) => {
  return (
      <li>
